...
export default class YieldFarmingDataStore {
    ...
    getPushRewardsDistributed = async () => {
        ...
        const currentEpochPUSH = await yieldFarmingPUSH.getCurrentEpoch();
        const genesisEpochAmountPUSH = ethers.BigNumber.from(
            this.state.genesisEpochAmountPUSH
        ).mul(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(18)));
        const deprecationPerEpochPUSH = ethers.BigNumber.from(
            this.state.deprecationPerEpochPUSH
        ).mul(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(18)));
        const currentEpochLP = await yieldFarmingLP.getCurrentEpoch();
        const genesisEpochAmountLP = ethers.BigNumber.from(
            this.state.genesisEpochAmountLP
        ).mul(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(18)));
        const deprecationPerEpochLP = ethers.BigNumber.from(
            this.state.deprecationPerEpochLP
        ).mul(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(18)));
